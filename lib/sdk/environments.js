...
exports.Environment = exports.EnvironmentClient = exports.WriteFileMode = void 0;
...
const pagination_pb_1 = require("../gitpod/v1/pagination_pb");
...
Object.defineProperty(exports, "WriteFileMode", { enumerable: true, get: function () { return environmentopts_pb_1.WriteMode; } });
...
class EnvironmentClient {
    ...
    async create(options) {
        ...
        if (!options.contextUrl) {
            throw new errors_1.ValidationError(operation, "context URL is required");
        }
        (0, logging_1.debug)(this.client.logger, "creating environment", {
            operation,
        ...
        if (projectId) {
            (0, logging_1.debug)(this.client.logger, "creating environment from project", {
                operation,
        ...
        (0, logging_1.debug)(this.client.logger, "selected environment class", {
            operation,
    ...
    async createScratch(options = {}) {
        ...
        const selected = await this.defaultEnvironmentClass(options.signal);
        const response = await this.client.services.environment.createEnvironment((0, protobuf_1.create)(environment_pb_1.CreateEnvironmentRequestSchema, {
            spec: (0, protobuf_1.create)(environment_pb_1.EnvironmentSpecSchema, {
                desiredPhase: environment_pb_1.EnvironmentPhase.RUNNING,
                machine: (0, protobuf_1.create)(environment_pb_1.EnvironmentSpec_MachineSchema, { class: selected.environmentClassId }),
            }),
            name: options.name,
        ...
        if (!response.environment) {
            throw new errors_1.CapabilityUnavailableError(operation, "create response did not include an environment");
        }
    ...
    async *list(options = {}) {
        ...
        const identity = await this.client.services.identity.getAuthenticatedIdentity((0, protobuf_1.create)(identity_pb_1.GetAuthenticatedIdentityRequestSchema), { signal: options.signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        const creatorId = identity.subject?.id;
        if (!creatorId) {
            throw new errors_1.CapabilityUnavailableError(operation, "authenticated identity did not include a subject ID");
        }
        let token = "";
        let count = 0;
        for (;;) {
            (0, logging_1.debug)(this.client.logger, "listing environments", {
                operation,
                creator_id: creatorId,
                page_size: options.pageSize ?? defaultPageSize,
                has_page_token: token !== "",
            });
            const response = await this.client.services.environment.listEnvironments((0, protobuf_1.create)(environment_pb_1.ListEnvironmentsRequestSchema, {
                pagination: (0, protobuf_1.create)(pagination_pb_1.PaginationRequestSchema, {
                    pageSize: options.pageSize ?? defaultPageSize,
                    token,
                }),
                filter: (0, protobuf_1.create)(environment_pb_1.ListEnvironmentsRequest_FilterSchema, {
                    creatorIds: [creatorId],
                    roles: [environment_pb_1.EnvironmentRole.DEFAULT],
                    archivalStatus: environment_pb_1.ListEnvironmentsRequest_ArchivalStatus.ALL,
                }),
            }), { signal: options.signal }).catch((error) => {
                throw (0, errors_1.mapError)(operation, error);
            });
            ...
            (0, logging_1.debug)(this.client.logger, "listed environment page", {
                operation,
                creator_id: creatorId,
    ...
    async stop(environmentId, options = {}) {
        ...
        (0, logging_1.debug)(this.client.logger, "stopping environment", { operation, environment_id: environmentId });
        await this.client.services.environment.stopEnvironment((0, protobuf_1.create)(environment_pb_1.StopEnvironmentRequestSchema, { environmentId }), { signal: options.signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        await this.waitForEnvironment(environmentId, operation, options.signal, (environment) => {
            const phase = environment.status?.phase;
            return phase === environment_pb_1.EnvironmentPhase.STOPPED || phase === environment_pb_1.EnvironmentPhase.DELETED;
        });
    }
    async delete(environmentId, options = {}) {
        ...
        (0, logging_1.debug)(this.client.logger, "deleting environment", {
            operation,
            environment_id: environmentId,
            force: options.force === true,
        });
        await this.client.services.environment.deleteEnvironment((0, protobuf_1.create)(environment_pb_1.DeleteEnvironmentRequestSchema, {
            environmentId,
            force: options.force === true,
        }), { signal: options.signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        (0, logging_1.debug)(this.client.logger, "environment deleted", {
            operation,
            environment_id: environmentId,
            force: options.force === true,
        });
    ...
    async start(environmentId, options = {}) {
        ...
        (0, logging_1.debug)(this.client.logger, "starting environment", { operation, environment_id: environmentId });
        await this.client.services.environment.startEnvironment((0, protobuf_1.create)(environment_pb_1.StartEnvironmentRequestSchema, { environmentId }), { signal: options.signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        const environment = await this.waitForEnvironment(environmentId, "environments.wait_running", options.signal, (current) => {
            return current.status?.phase === environment_pb_1.EnvironmentPhase.RUNNING;
        });
    ...
    async getHandle(environmentId, signal) {
        ...
        const response = await this.client.services.environment.getEnvironment((0, protobuf_1.create)(environment_pb_1.GetEnvironmentRequestSchema, { environmentId }), { signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        if (!response.environment) {
            throw new errors_1.NotFoundError(operation, `environment ${environmentId} was not returned`);
        }
    ...
    async environment(handle, waitForRunning, signal) {
        if (waitForRunning) {
            const latest = await this.waitForEnvironment(handle.id, "environments.wait_running", signal, (environment) => {
                return environment.status?.phase === environment_pb_1.EnvironmentPhase.RUNNING;
            });
    ...
    async waitForEnvironment(environmentId, operation, signal, done) {
        ...
        if (operation === "environments.wait_running" && isTerminalBeforeRunning(current.status?.phase)) {
            throw new errors_1.UnavailableError(operation, `environment ${environmentId} reached ${environmentPhaseLabel(current.status?.phase)} before running`);
        }
        const events = this.client.services.event.watchEvents((0, protobuf_1.create)(event_pb_1.WatchEventsRequestSchema, {
            ...
            resourceTypeFilters: [
                (0, protobuf_1.create)(event_pb_1.WatchEventsRequest_ResourceTypeFilterSchema, {
                    resourceType: event_pb_1.ResourceType.ENVIRONMENT,
                    resourceIds: [environmentId],
                }),
            ],
        }), { signal });
        for await (const event of events) {
            if (event.resourceId !== environmentId || event.resourceType !== event_pb_1.ResourceType.ENVIRONMENT) {
                continue;
            }
        ...
        throw new errors_1.UnavailableError(operation, `environment ${environmentId} event stream ended before the target phase`);
    }
    async resolveContext(contextUrl, signal) {
        const operation = "scm.resolve_context";
        (0, logging_1.debug)(this.client.logger, "resolving context URL", {
            operation,
            context_url: safeURLForLog(contextUrl),
        });
        const response = await this.client.services.runner.parseContextURL((0, protobuf_1.create)(runner_pb_1.ParseContextURLRequestSchema, { contextUrl }), { signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        (0, logging_1.debug)(this.client.logger, "resolved context URL", {
            operation,
    ...
    async requireSCMAuthentication(runnerId, contextUrl, signal) {
        ...
        const response = await this.client.services.runner.checkAuthenticationForHost((0, protobuf_1.create)(runner_pb_1.CheckAuthenticationForHostRequestSchema, { runnerId, host }), { signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        if (!response.authenticated) {
            (0, logging_1.debug)(this.client.logger, "SCM authentication required", {
                operation,
                runner_id: runnerId,
                host,
                scm_id: response.scmId,
            ...
            throw new errors_1.AuthenticationRequiredError(operation, `authentication required for ${host}`);
        }
        (0, logging_1.debug)(this.client.logger, "SCM authentication available", {
            operation,
            runner_id: runnerId,
            host,
            scm_id: response.scmId,
    ...
    async defaultEnvironmentClass(signal) {
        ...
        const response = await this.client.services.environment.listEnvironmentClasses((0, protobuf_1.create)(runner_configuration_pb_1.ListEnvironmentClassesRequestSchema, {
            pagination: (0, protobuf_1.create)(pagination_pb_1.PaginationRequestSchema, { pageSize: defaultPageSize }),
            filter: (0, protobuf_1.create)(runner_configuration_pb_1.ListEnvironmentClassesRequest_FilterSchema, {
                enabled: true,
        ...
        if (!selected) {
            throw new errors_1.CapabilityUnavailableError(operation, "no environment class available for creating environments");
        }
        (0, logging_1.debug)(this.client.logger, "selected default environment class", {
            operation,
    ...
    async environmentClassRunnerID(environmentClassId, signal) {
        ...
        const response = await this.client.services.runnerConfiguration.getEnvironmentClass((0, protobuf_1.create)(runner_configuration_pb_1.GetEnvironmentClassRequestSchema, { environmentClassId }), { signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        const runnerId = response.environmentClass?.runnerId ?? "";
        if (!runnerId) {
            throw new errors_1.CapabilityUnavailableError(operation, `environment class ${environmentClassId} did not include a runner ID`);
        }
        return runnerId;
    }
    async preferredProjectID(projectIds, signal) {
        ...
        const now = new Date();
        const response = await this.client.services.usage.getTopProjects((0, protobuf_1.create)(usage_pb_1.GetTopProjectsRequestSchema, {
            pagination: (0, protobuf_1.create)(pagination_pb_1.PaginationRequestSchema, { pageSize: defaultPageSize }),
            dateRange: (0, protobuf_1.create)(usage_pb_1.DateRangeSchema, {
                startTime: (0, wkt_1.timestampFromDate)(new Date(now.getTime() - defaultProjectUsageLookbackMs)),
                endTime: (0, wkt_1.timestampFromDate)(now),
            }),
        }), { signal }).catch(() => undefined);
        for (const project of response?.projects ?? []) {
            if (candidates.has(project.projectId)) {
                (0, logging_1.debug)(this.client.logger, "selected project for context URL", {
                    operation,
    ...
    async createFromContextURL(contextUrl, environmentClass, name, signal) {
        ...
        const response = await this.client.services.environment.createEnvironment((0, protobuf_1.create)(environment_pb_1.CreateEnvironmentRequestSchema, {
            spec: (0, protobuf_1.create)(environment_pb_1.EnvironmentSpecSchema, {
                desiredPhase: environment_pb_1.EnvironmentPhase.RUNNING,
                machine: environmentClass ? (0, protobuf_1.create)(environment_pb_1.EnvironmentSpec_MachineSchema, { class: environmentClass }) : undefined,
                content: (0, protobuf_1.create)(environment_pb_1.EnvironmentSpec_ContentSchema, {
                    initializer: (0, protobuf_1.create)(environment_pb_1.EnvironmentInitializerSchema, {
                        specs: [
                            (0, protobuf_1.create)(environment_pb_1.EnvironmentInitializer_SpecSchema, {
                                spec: {
                                    case: "contextUrl",
                                    value: (0, protobuf_1.create)(environment_pb_1.ContextURLInitializerSchema, { url: contextUrl }),
                                },
                            }),
                        ],
                    }),
    ...
    async createFromProject(projectId, resolved, name, signal) {
        ...
        const response = await this.client.services.environment.createEnvironmentFromProject((0, protobuf_1.create)(environment_pb_1.CreateEnvironmentFromProjectRequestSchema, {
            projectId,
        ...
        if (!response.environment) {
            throw new errors_1.CapabilityUnavailableError(operation, "create-from-project response did not include an environment");
        }
        return handleFor(response.environment);
    }
    async projectRemoteURIForParsedContext(projectId, gitContext, signal) {
        ...
        const response = await this.client.services.project.getProject((0, protobuf_1.create)(project_pb_1.GetProjectRequestSchema, { projectId }), { signal }).catch(() => undefined);
...
class Environment {
    ...
    async runCommand(options) {
        ...
        if (!options.command) {
            throw new errors_1.ValidationError(operation, "command is required");
        }
        const ops = await this.opsClient(options.signal);
        const operationId = randomID();
        (0, logging_1.debug)(this.client.logger, "running command in environment", {
            operation,
        ...
        const response = await ops.exec((0, protobuf_1.create)(environmentopts_pb_1.ExecRequestSchema, {
            operationId,
        ...
        (0, logging_1.debug)(this.client.logger, "environment command finished", {
            operation,
            environment_id: this.id(),
            operation_id: operationId,
    ...
    async readFile(path, options = {}) {
        ...
        if (!path) {
            throw new errors_1.ValidationError(operation, "path is required");
        }
        const ops = await this.opsClient(options.signal);
        const response = await ops.readFile((0, protobuf_1.create)(environmentopts_pb_1.ReadFileRequestSchema, {
            path,
    ...
    async writeFile(path, content, options = {}) {
        ...
        const ops = await this.opsClient(options.signal);
        const response = await ops.writeFile((0, protobuf_1.create)(environmentopts_pb_1.WriteFileRequestSchema, {
            path,
            content: data,
            mode: options.mode ?? environmentopts_pb_1.WriteMode.UNSPECIFIED,
        }), { signal: options.signal }).catch((error) => {
    ...
    async gitChanges(options = {}) {
        ...
        for (const file of status.changedFiles) {
            const diff = await ops.getGitDiff((0, protobuf_1.create)(environmentopts_pb_1.GetGitDiffRequestSchema, {
                path: file.path,
    ...
    startCodex(options) {
        return (0, agents_1.startCodexInEnvironment)(this.client, this.id(), options);
    }
    async gitStatus(signal) {
        const ops = await this.opsClient(signal);
        const response = await ops.getGitStatus((0, protobuf_1.create)(environmentopts_pb_1.GetGitStatusRequestSchema), { signal }).catch((error) => {
            throw this.mapOpsError("ops.git_status", error);
        });
        if (!response.status) {
            throw new errors_1.CapabilityUnavailableError("ops.git_status", "git status response did not include status");
        }
    ...
    async opsClient(signal) {
        ...
        if (!opsUrl) {
            throw new errors_1.CapabilityUnavailableError(operation, "environment ops URL is not available");
        }
        const token = await this.client.services.environment.createEnvironmentAccessToken((0, protobuf_1.create)(environment_pb_1.CreateEnvironmentAccessTokenRequestSchema, { environmentId: this.id() }), { signal }).catch((error) => {
            throw (0, errors_1.mapError)(operation, error);
        });
        if (!token.accessToken) {
            throw new errors_1.CapabilityUnavailableError(operation, "environment access token response did not include an access token");
        }
    ...
    mapOpsError(operation, error) {
        if (isEnvironmentOpsReachabilityError(error)) {
            ...
            return new errors_1.EnvironmentUnreachableError(operation, `cannot reach environment ${this.id()} ops service at ${safeURLForLog(opsUrl)}: ${(0, errors_1.messageOf)(error)}`, error);
        }
        return (0, errors_1.mapError)(operation, error);
    }
}
...
function environmentSpecFromProject(resolved, gitRemoteURI) {
    ...
    return (0, protobuf_1.create)(environment_pb_1.EnvironmentSpecSchema, {
        content: (0, protobuf_1.create)(environment_pb_1.EnvironmentSpec_ContentSchema, {
            initializer: (0, protobuf_1.create)(environment_pb_1.EnvironmentInitializerSchema, {
                specs: [
                    (0, protobuf_1.create)(environment_pb_1.EnvironmentInitializer_SpecSchema, {
...
function gitInitializerFromParsedContext(gitContext, remoteURI) {
    ...
    let targetMode = environment_pb_1.GitInitializer_CloneTargetMode.REMOTE_COMMIT;
    if (!target) {
        target = gitContext.tag;
        targetMode = environment_pb_1.GitInitializer_CloneTargetMode.REMOTE_TAG;
    }
    if (!target) {
        target = gitContext.branch;
        targetMode = environment_pb_1.GitInitializer_CloneTargetMode.REMOTE_BRANCH;
    }
    if (!target) {
        return undefined;
    }
    return (0, protobuf_1.create)(environment_pb_1.GitInitializerSchema, {
...
function hostFromURL(rawURL) {
    try {
        ...
        throw new errors_1.ValidationError("environments.require_scm_authentication", `parse context URL: ${(0, errors_1.messageOf)(error)}`, error);
    }
}
...
function environmentFailureError(operation, environment) {
    ...
    return new errors_1.EnvironmentPolicyError(operation, `environment ${environment.id} failed: ${failures.join("; ")}`);
}
...
function environmentPhaseLabel(phase) {
    switch (phase) {
        case environment_pb_1.EnvironmentPhase.CREATING:
            return "creating";
        case environment_pb_1.EnvironmentPhase.STARTING:
            return "starting";
        case environment_pb_1.EnvironmentPhase.RUNNING:
            return "running";
        case environment_pb_1.EnvironmentPhase.UPDATING:
            return "updating";
        case environment_pb_1.EnvironmentPhase.STOPPING:
            return "stopping";
        case environment_pb_1.EnvironmentPhase.STOPPED:
            return "stopped";
        case environment_pb_1.EnvironmentPhase.DELETING:
            return "deleting";
        case environment_pb_1.EnvironmentPhase.DELETED:
            return "deleted";
        default:
            return "unspecified";
    }
...
function logEnvironmentPhase(client, operation, environment) {
    (0, logging_1.debug)(client.logger, "environment phase changed", {
        operation,
...
function isEnvironmentOpsReachabilityError(error) {
    const message = (0, errors_1.messageOf)(error).toLowerCase();
